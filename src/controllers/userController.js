...
const userController = {
    ...
    deleteUser: async (req,res) => {
        try {
            const user_email = req.body.data.email;
            if(!user_email) return res.status(400).json('missing user email');
            const deleteUser = await userService.deleteUser(user_email);
            if(deleteUser) return res.status(200).json('suceess');
        } catch (error) {
            console.log(error);
            return res.status(500).json({'error: ': error.message})
        }
    },
    deleteUserV2: async (req,res) => {
        try {
